...
BUILD_ESSENTIAL="apt-get -y update && \
	apt-get -y install git wget build-essential texinfo python3 bison zlib1g-dev libtool cmake gcc g++ libc6 gawk"
...
if [ ! -f ${PWD_PATH}/succeed/gcc ]; then
	...
	--disable-checking   \
        --program-suffix=-8.1 \
	--disable-lto \
	--enable-multilib --with-arch=armv7-a --with-fpu=vfpv3-d16 --with-float=hard --with-system-zlib --with-target-system-zlib=auto --disable-bootstrap \
...
if [ ! -f ${PWD_PATH}/succeed/clang ]; then
    ...
    cd build_clang
    cmake ${LLVM_PATH} -DCMAKE_INSTALL_PREFIX=$LLVM_EXE_DIR -DCMAKE_EXE_LINKER_FLAGS_DEBUG="-I/usr/local/include -L/usr/local/lib -lprotobuf -lpthread" -DLLVM_ENABLE_RTTI=ON -DLLVM_BINUTILS_INCDIR=$BINUTILS_PATH/include -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD="ARM"
