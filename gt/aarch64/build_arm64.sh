...
PWD_PATH="${SRC_PATH}/build_aarch64"
...
BUILD_ESSENTIAL="apt-get -y update && \
	apt-get -y install git wget build-essential python3 texinfo bison zlib1g-dev libtool cmake gcc g++ libc6 gawk"
...
if [ ! -f ${PWD_PATH}/succeed/protobuf ]; then
    ...
    eval "./configure && make -j8 &&  make install && ldconfig"
...
if [ ! -f ${PWD_PATH}/succeed/protobuf-c ]; then
    ...
    eval 'cd protobuf-c && ./autogen.sh && ./configure && make -j8 && make install'
...
if [ ! -f ${PWD_PATH}/succeed/gcc ]; then
    ...
    cp $SRC_PATH/libtool.m4 $GCC_PATH
	...
	--disable-libsanitizer \
        --enable-languages=c,c++,fortran && make -j8"
    ...
    cp -r "${SRC_PATH}/gcc/gcc-8.1.0/gcc" ${GCC_PATH}/gcc
      ...
      # gcc
    eval "cd $GCC_BUILD_PATH && make -j8 &&   make install"
...
if [ ! -f ${PWD_PATH}/succeed/clang ]; then
    ...
    LLVM_EXE_DIR=${PWD_PATH}/executable_clang
    cd build_clang
    cmake ${LLVM_PATH} -DCMAKE_INSTALL_PREFIX=$LLVM_EXE_DIR -DCMAKE_EXE_LINKER_FLAGS_DEBUG="-I/usr/local/include -L/usr/local/lib -lprotobuf -lpthread" -DLLVM_ENABLE_RTTI=ON -DLLVM_BINUTILS_INCDIR=$BINUTILS_PATH/include -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD="AArch64"
...
if [ ! -f ${PWD_PATH}/succeed/glibc ]; then
    ...
    ENV64=${SRC_PATH}/gcc64_arm_build.rc
    ENV32=${SRC_PATH}/gcc32_arm_build.rc
    ...
    export CFLAGS="-O2 ${CFLAGS} -Wno-error -fPIC"
    export CXXFLAGS="-O2 ${CXXFLAGS} -Wno-error -fPIC"
