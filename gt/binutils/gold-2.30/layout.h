...
class Layout
{
 public:
  ...
  reset_relax_output();
	...
	Output_section* get_output_section_by_name(const char* sec_name);
	...
	void insert_obj2input(Sized_relobj_file<size, big_endian>* obj, 
	  ...
	  //gold_info("[bbinfo]: adding the input2output record. %s -> %s", input_sec_name.c_str(), obj->name().c_str());
  ...
  std::map<Relobj*, bool> get_relobj_map(){
    return relobj_map;
  }
  
  void insert_relobj(Relobj* obj){
  ...
  // Has handled this object, so set the key to true
 ...
 private:
  ...
  std::map<Relobj*, bool>  relobj_map; // this record all relobj. key(relobj) => value(handle its .rand or not)
};
