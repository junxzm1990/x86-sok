    ...
    section_size_type reloc_view_size)
{
  ...
  printf("T:object is %s section is %s\n",object->name().c_str(),object->section_name(relinfo->data_shndx).c_str());

//   bool find_arm_idx = true;
//   char T_arm_idx[] = ".ARM.exidx";

//   for(int T = 0;T < 10 && T < object->section_name(relinfo->data_shndx).length();T++)
// 	  if(T_arm_idx[T] != object->section_name(relinfo->data_shndx)[T])
// 	  {
