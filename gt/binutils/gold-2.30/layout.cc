...
// typedef std::unordered_map<uint32_t, ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple*>::iterator OffsetFixupMap
...
//ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple* bbinfo_tmp_fixup;
  uint32_t fixup_text_cur_idx = 0;
  uint32_t fixup_rodata_cur_idx = 0;
  uint32_t fixup_data_cur_idx = 0;
  uint32_t fixup_datarel_cur_idx = 0;
  uint32_t fixup_initarray_cur_idx = 0;
	       ...
	       unsigned int reloc_shndx, unsigned int, off_t* off)
{
  *off = 0;
  ...
  if(!strcmp(name, ".rand")){
    ...
    //gold_info("[CCR] in %s found .rand", object->name().c_str());
...
Layout::reset_relax_output()
{
  ...
  this->relax_output_list_.clear();
    ...
    uint32_t get_fixup_current_idx(std::string sec_name) {
      switch (get_fixup_section_id(sec_name)) {
            case 0: return fixup_text_cur_idx;
            case 1: return fixup_rodata_cur_idx;
            case 2: return fixup_data_cur_idx;
            case 3: return fixup_datarel_cur_idx;
            case 4: return fixup_initarray_cur_idx;
            default: gold_unreachable();
        }
    }

    void set_fixup_current_idx(std::string sec_name, uint32_t number) {
      switch (get_fixup_section_id(sec_name)) {
            case 0: fixup_text_cur_idx = number; break;
            case 1: fixup_rodata_cur_idx = number; break;
            case 2: fixup_data_cur_idx = number; break;
            case 3: fixup_datarel_cur_idx = number; break;
            case 4: fixup_initarray_cur_idx = number; break;
            default: gold_unreachable();
        }
    }
    ...
    update_fixup_type(ShuffleInfo::ReorderInfo_FixupInfo* p_fixup_info, std::string sec_name, int fixup_offset,
          ...
          uint32_t current_fixup_idx = get_fixup_current_idx(sec_name);
          uint32_t saved_fixup_idx = current_fixup_idx;
          uint32_t fixup_size = get_fixup_size(p_fixup_info, sec_name);
          do{
            ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple* p_fixup_tuple = get_mutable_fixup(p_fixup_info, sec_name, current_fixup_idx);
            current_fixup_idx = (current_fixup_idx + 1) % fixup_size;
            if(p_fixup_tuple->offset() == (uint32_t)fixup_offset) {
              ...
              set_fixup_current_idx(sec_name, current_fixup_idx);
              return;
            }
          }while(saved_fixup_idx != current_fixup_idx);
        return;
    }
...
// 	    // binpang. It cost a lot of time when the project is too big. So I comment it out for now.
// 	    // FIXME. Improve the spped.
...
// #ifdef CCR_MSG_DETAILS
//                 gold_info("[bbinfo] WARNING: don't record the fixups in sections %s", sec_name.c_str());
// #endif
//                 return;
//             }
...
// #ifdef CCR_MSG_DETAILS
//                 gold_info("[bbinfo] WARNING: can't find the finxup which offset 0x%x in section %s",
...
*/
    ...
    // binpang, update the section boundary.
    void Layout::update_section_boundary(){
      ShuffleInfo::ReorderInfo& gsi = get_global_shuffleInfo();

      typedef struct{
	uint64_t offset; // layout offset
	uint32_t index; // layout index
      } layout_element;

      uint32_t i;
      std::map<std::string, layout_element> section_map;
      std::map<std::string, layout_element>::iterator cur_iter;
      for (i = 0; i < gsi.layout_size(); i++){
	std::string sec_name = gsi.layout(i).section_name();
	cur_iter = section_map.find(sec_name);
	if (cur_iter != section_map.end()){
	  // if there exists the section record, check if the offset size
	  if (cur_iter->second.offset < gsi.layout(i).offset()){
	    cur_iter->second.offset = gsi.layout(i).offset();
	    cur_iter->second.index = i;
	  }
	}
	// the section not in the map 
       	else {
	  layout_element tmp_layout{gsi.layout(i).offset(), i};
	  section_map.insert(std::pair<std::string, layout_element>(sec_name, tmp_layout));
	}
      }
	...
	if (NORMAL_BBL == gsi.layout(index).type()){
	  ...
	  gsi.mutable_layout(index)->set_type(OBJ_FUNC_BBL);
	}
      }
    }
    
    // get the output section
    Output_section* Layout::get_output_section_by_name(const char* sec_name){
      for (Section_list::const_iterator p = this->section_list_.begin();
	  p != this->section_list_.end(); ++p){
	Output_section* os = *p;
	if (!strcmp(os->name(), sec_name))
	  return os;
      }
#ifdef CCR_MSG_DETAILS
      gold_info("[bbinfo]: ERROR. The text section %s does not in output section list!",
	  sec_name);
#endif 
    }
    ...
    void Layout::update_shuffleInfo_layout(){
        ...
        for (Section_list::const_iterator p = this->section_list_.begin(); p != this->section_list_.end(); ++p)
        {
	      ...
	      const unsigned char *my_contents = obj2->section_contents(obj2->get_rand_shndx(".rand"), &rand_sec_len2, false);
	      std::string rand_input(reinterpret_cast<const char *>(my_contents), rand_sec_len2);
	      if(rand_sec_len2 > 0){
        ...
        for (i = 0; i < to_add_info.layout_size(); ++i) {
          ...
          if(!pfrom_object->yarp_text_section_exist(cur_sec_name)) {
              ...
              gold_info("[CCR] Discard %s:%s BBL#[%3d], accu redundant section size: %lx,Total removed fixups: %2d (0x%xB)",
                        pfrom_object->name().c_str(), to_add_info.layout(j).section_name().c_str(), j,
		       	redundant_section_size, accum_removed_child_fixups, redundant_section_size);
#endif
	      continue;
          } // binpang, add.
          ...
          if (i < to_add_info.layout_size()){
              ...
              // binpang. For relocatable object, we reseting the type of `OBJECT`
              if (parameters->options().relocatable()){
                int tmp_type = to_add_info.layout(i).type();
                if (OBJ_BBL == tmp_type || NORMAL_BBL == tmp_type){
                  playoutinfo->set_type(NORMAL_BBL);
                } else if (FUNC_BBL == tmp_type || OBJ_FUNC_BBL){
                  playoutinfo->set_type(FUNC_BBL);
                }
              } else{
		 ...
		 Target* target, const Task* task)
{
  ...
  // binpang.  Get the target binary information: [size, is_big_endian]
  bool is_big_endian = parameters->target().is_big_endian();
  int binary_format_size = parameters->target().get_size();
  gold_info("[bbinfo]: DEBUG, the target binary format is: size %d, is big endian %d", \
            binary_format_size, is_big_endian);
  if (is_big_endian && binary_format_size == 64){
    ...
    gold_info("[bbinfo]: WARNING, Not support the binary format. Size is %d, is big endian %d", \
              binary_format_size, is_big_endian);
  }

  // binpang. If the output object is `relocatable`, update the section boundary.
  if (parameters->options().relocatable()){
    this->update_section_boundary();
  }
  // this->update_shuffleInfo_layout<64, false>();
  // ShuffleInfo::ReorderInfo& reorder_info = this->get_global_shuffleInfo();
  // if (reorder_info.fixup_size() > 0){
  //   ShuffleInfo::ReorderInfo_FixupInfo* p_fixup_info = reorder_info.mutable_fixup(0);
