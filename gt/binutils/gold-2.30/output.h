...
class Output_section : public Output_data
{
 public:
   ...
   // binpang, add. BEGIN
   uint64_t output_yarp_object_id;
   // map: obj_id => prev_objtext_pading_size
   std::map<uint64_t, uint64_t> output_yarp_obj_text_padding_map;
   std::set<uint64_t> output_zero_obj_ids;

   uint64_t output_get_and_increase_yarp_obj_id(){
     return output_yarp_object_id++;
   }
   void output_reset_yarp_obj_id(){
     output_yarp_object_id = 0;
   }

   void 
   output_set_yarp_obj_text_padding_map(uint64_t id, uint64_t prev_padding_size){
     output_yarp_obj_text_padding_map[id] = prev_padding_size;
   }
   void output_reset_yarp_obj_text_padding_map(){
     output_yarp_obj_text_padding_map.clear();
   }

   uint64_t
   output_get_yarp_obj_text_padding_map(int64_t id){
     int32_t padding_size = 0;
     uint64_t next_id = id;
     if (id < 0 || output_is_zero_object(id))
       return 0;
     else{
       do{
	 next_id += 1;
	 padding_size += output_yarp_obj_text_padding_map[next_id];
       }while(output_is_zero_object(next_id));
       return padding_size;
     }
   }

   bool output_is_zero_object(uint64_t obj_id){
     return output_zero_obj_ids.find(obj_id) != output_zero_obj_ids.end();
   }

   void output_record_zero_object(uint64_t obj_id){
     output_zero_obj_ids.insert(obj_id);
   }

   void output_reset_zero_object_set(){
     output_zero_obj_ids.clear();
   }
   // binpang, add. END
