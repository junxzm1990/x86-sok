...
Output_section::Output_section(const char* name, elfcpp::Elf_Word type,
  ...
  : name_(name),
    ...
    patch_space_(0),
    output_yarp_object_id(0) // binpang, add. record the object id.
{
				  ...
				  bool have_sections_script)
{
  ...
  // binpang, add current object to relobj_map
  layout->insert_relobj((Relobj*)object);
        ...
        if(is_prefix_of(".text", secname))
        {
          ...
          uint64_t obj_id = this->output_get_and_increase_yarp_obj_id();
...
#ifdef CCR_MSG_DETAILS
          gold_info("[lala]setting object_id_map[%s->%d] for %s in output section [%s]", secname, obj_id, object->name().c_str(), this->name());
#endif
          object->set_yarp_object_id_map(obj_id, secname);
          this->output_set_yarp_obj_text_padding_map(obj_id, aligned_offset_in_section-offset_in_section);
	      ...
	      this->output_record_zero_object(obj_id);
...
Output_section::set_final_data_size()
{
  ...
  if (this->input_sections_.empty())
	   ...
	   ++p)
	{
      ...
      if (yarp_has_reorder && is_prefix_of(".text", this->name()) && p->is_input_section()){
	    ...
	    uint64_t obj_id = this->output_get_and_increase_yarp_obj_id();
#ifdef CCR_MSG_DETAILS
	    gold_info("[lala]repopulating object_id_map[%s->%d] (total: %d) for %s[%p] in output section [%s]", 
	    insecname, obj_id, p->relobj()->get_yarp_obj_id_map().size(), p->relobj()->name().c_str(), p->relobj(), this->name());
#endif
	    p->relobj()->set_yarp_object_id_map(obj_id, insecname);
#ifdef CCR_MSG_DETAILS
	    gold_info("[lala]recomputing %s section: %s", p->relobj()->name().c_str(), insecname);
	    gold_info("\t\tbefore  off: %x, size: %x, req: %x", off + startoff, p->data_size(), p->addralign());
#endif
	    off_t aligned_off = align_address(off + startoff, p->addralign());
#ifdef CCR_MSG_DETAILS
	    gold_info("\t\tafter   off: %x, aligned off: %x, off: %x, size: %x, padding: %x, prev raw: %x",
		  aligned_off , aligned_off, off + startoff ,p->data_size(), prev_raw_off?aligned_off-prev_raw_off:0, prev_raw_off);
#endif
	    //my_layout->set_yarp_obj_text_padding_map(obj_id, prev_raw_off?aligned_off - prev_raw_off:0);
	    this->output_set_yarp_obj_text_padding_map(obj_id, prev_raw_off?aligned_off - prev_raw_off:0);
	    if(p->data_size() == 0){
...
Output_section::sort_attached_input_sections()
{
  ...
  if (this->must_sort_attached_input_sections())
	...
	std::sort(sort_list.begin(), sort_list.end(),
      ...
      this->output_reset_yarp_obj_id();
      this->output_reset_zero_object_set();
      this->output_reset_yarp_obj_text_padding_map();
