...
def compareJmpTables(groundTruth, compared, insts):
    ...
    for (terminator, successors) in compared.items():
        if terminator not in groundTruth and (terminator + 0x4) not in groundTruth:
            compared_result = ", ".join(f"0x{x:x}" for x in successors)
            logging.error("[Instruction False Positive #%d]Instruction address %x not in groudtruth. Compared is %s" %
                    (falsePositive, terminator, compared_result))
            falsePositive += 1
            continue
        if terminator not in groundTruth and (terminator + 0x4) in groundTruth:
            logging.warning("WTF")
            terminator = terminator + 0x4
        ...
        if compare_result < 0:
            ...
            for suc in successors:
                if suc not in groundTruth[terminator]:
                    cnt = cnt + 1
                    logging.error("FalsePositive Successor #%d: 0x%x" %(cnt,suc))
            if cnt == 1:
                truePositive += 1
        else:
            logging.error("[Less than ground truth]: 0x%x" % terminator)
            cnt = 0
            for suc in groundTruth[terminator]:
                if suc not in successors:
                    cnt = cnt + 1
                    logging.error("FalseNegative Successor #%d: 0x%x" %(cnt,suc))

    ## compute the false negative number
    exclude_num = 0
    for (terminator, successors) in groundTruth.items():
        if terminator not in compared and terminator - 0x4 not in compared:
...
def readJmpTablesCompared(mModule, binary, groundTruth,ida_flag = False):
    """
    read jump tables from protobufs
    params:
        mModule: protobuf module
    ...
    for func in mModule.fuc:
        ...
        for bb in func.bb:
            ...
            for inst in bb.instructions:
                inst_addr = inst.va - disassembler_base_addr
                if inst_addr % 2 == 1 and angr_flag:
                    inst_addr = inst_addr - 1
                insts.add(inst_addr)
            terminator_addr = bb.instructions[-1].va
            if ida_flag and terminator_addr != bb.instructions[0].va:
                continue
            if PIE:
            ...
            if terminator_addr in groundTruth:
                ...
                successors = set()
                for suc in bb.child:
                    succ = suc.va - disassembler_base_addr
                    if not isInTextSection(succ):
                        continue
                    if succ % 2 == 1 and angr_flag:
                        succ = succ - 1
                    successors.add(succ)
                ...
                if (bb.instructions[0].va + bb.size) in successors:
                    successors.remove(bb.instructions[0].va + bb.size)
                successors_result = ", ".join(f"0x{x:x}" for x in successors)
                if terminator_addr % 2 == 1 and angr_flag:
                    terminator_addr = terminator_addr - 1
                logging.debug("[Indirect jump at 0x%x: successors is %s]" % (terminator_addr, successors_result))
                result[terminator_addr] = successors
    return (result, insts)
def readInstsCompared(mModule):
    """
    read jump tables from protobufs
    params:
        mModule: protobuf module
...
if __name__ == '__main__':
    ...
    parser.add_option("-i", "--instructionfile", dest = "instructionfile", action = "store", \
        type = "string", help = "ghidra cfg file path", default = None)
    ...
    assert options.instructionfile != None, "Please input the instruction file!"
    ...
    try:
        ...
        f2.close()
        f3 = open(options.instructionfile, 'rb')
    ...
    if "ghidra" in options.comparedfile.lower() and PIE:
        doubleCheckGhidraBase(mModule3)
    ...
    insts = readInstsCompared(mModule3)
