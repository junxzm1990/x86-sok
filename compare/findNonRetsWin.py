...
KNOWN_NON_RETS = {"exit", "abort", "__f90_stop", "fancy_abort", "__stack_chk_fail",
        ...
        "_gfortran_exit_i4", "for_stop_core", "__sys_exit", "_Exit", "ExitThread", "FatalExit", "RaiseException", "RtlRaiseException", "_exit", "terminate"
        }
...
def parseDirectCall(MD, content, va, call_type):
    ...
    try:
        ...
        return (None, None, cur_inst)

    if x86.X86_GRP_CALL not in cur_inst.groups:
        return (None, None, cur_inst)

    if call_type == 1 and isPureIndirect(cur_inst):
        return (None, None, cur_inst)


    target_result = getDirectTargetAndMem(cur_inst)

    return (target_result[0], target_result[1], cur_inst)
...
def parseDirectJump(MD, content, va):
    disasm_ins = MD.disasm(content, va, count = 1)
    ...
    if x86.X86_GRP_JUMP not in cur_inst.groups:
        return None
...
def reconstructStructs(mModule, known_non_rets, binary, iat_targets):
    visited = set()
    ...
    for func in mModule.fuc:
        ...
        for bb in func.bb:
            if bb.type == BlockType.NON_RETURN_CALL:
                ...
                if bb.child[0].va in known_non_rets:
                    continue

                MD = init_capstone(ELFCLASS)

                inst_va = bb.instructions[-1].va
                offset = getOffset(inst_va)
                end_offset = offset + 0x20 if offset + 0x20 < content_len else content_len

                # double check
                (target, _, ret_type) = tryGetDirectTarget(MD, content[offset: end_offset], inst_va, x86.X86_GRP_CALL, iat_targets)

                if target != bb.child[0].va:
                    continue

                logging.debug("adding non-return from compiler. 0x%x at address 0x%x, func 0x%x" % (bb.child[0].va, bb.instructions[-1].va, func.va))
    ...
    for func in mModule.fuc:
        ...
        for bb in func.bb:
            ...
            MD = init_capstone(ELFCLASS)
            if bb.terminate == True:
                contains_terminate_funcs.add(cur_func)
                cons_bb.setTerminate()
                logging.debug("add terminate bb at 0x%x" % bb.va)
            ...
            for inst in bb.instructions:
                ...
                end_offset = offset + 0x20
                (target, _, ret_type) = tryGetDirectTarget(MD, content[offset: end_offset], inst.va, x86.X86_GRP_CALL, iat_targets)
                if not target:
                    continue

                if ret_type != None and ret_type == 0x2 and inst.va == bb.instructions[-1].va:
                    bb.child.add().va = target
            ...
            exclude_target = -1
            last_inst_va = bb.instructions[-1].va

            offset = getOffset(inst.va)
            end_offset = offset + bb.instructions[-1].size
            logging.debug("offset 0x%x, end_offset 0x%x" % (offset, end_offset))
            (target, inst_cap, _) = tryGetDirectTarget(MD, content[offset: end_offset], inst.va, x86.X86_GRP_JUMP, iat_targets)

            if inst_cap != None and x86.X86_GRP_RET in inst_cap.groups:
                cons_bb.setRet()
            ...
            if target in identified_funcs or target in iat_targets:
                logging.debug("reset tail call at 0x%x, child is 0x%x" % (last_inst_va, target))
                if 'jmp' in inst_cap.mnemonic:
                    del bb.child[:]
            ...
            logging.debug("current bb is 0x%x, type is 0x%x, last inst is 0x%x" % (bb.va, bb.type, bb.instructions[-1].va))
            if bb.type in {BlockType.DIRECT_CALL, BlockType.TAIL_CALL}:
                max_dis = 0
                for suc in bb.child:
                    if abs(suc.va - last_inst_va) > max_dis:
                        exclude_target = suc.va
                        max_dis = abs(suc.va - last_inst_va)
            ...
            logging.debug("current basic block is 0x%x" % bb.va)
            for suc in bb.child:
                if suc.va == exclude_target:
                    continue
                logging.debug("append the successor 0x%x" % suc.va)
    ...
    while len(queue) != 0:
        ...
        for (cur_addr, called_func_addr) in cur_called_funcs:
            ...
            if called_func_addr in visited_funcs:
                logging.debug("cyclic_dep at 0x%x" % func.va)
                cyclic_dep = True
                continue
        ...
        logging.debug("current basic block is 0x%x, called non-return function %d, is exitnode %d, type is %d, successors is %d" % 
                (cur_bb.va, has_called_non_ret, isExitNode, cur_bb.type, len(cur_bb.successors_addr)))
        ...
        if has_called_non_ret:
            if not (cur_bb.type in {BlockType.TAIL_CALL} and len(cur_bb.successors_addr) > 0):
                continue
...
def findNonRets(func_map, bb_map, called_edges, known_nonret_funcs, contains_terminate_funcs, iat_nonret_funcs):
    ...
    all_added_funcs = all_added_funcs.union(iat_nonret_funcs)
    while True:
...
if __name__ == '__main__':
    ...
    if not IS_PE:
        ...
        iats = getNonRetFuncsFromImportObjs(options.binaryfile, known_nonret_funcs)

    iat_known_nonret_funcs = iat_known_nonret_funcs.union(known_nonret_funcs)

    (func_map, bb_map, called_edges, contains_terminate_funcs) = \
            reconstructStructs(module, known_nonret_funcs, options.binaryfile, iats)

    (valid_nonret_funcs, all_added_funcs) = findNonRets(func_map, bb_map, called_edges, known_nonret_funcs, contains_terminate_funcs, iat_known_nonret_funcs)
