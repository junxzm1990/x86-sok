...
def compareJmpTables(groundTruth, compared, insts):
    ...
    for (terminator, successors) in compared.items():
        if terminator not in groundTruth:
...
if __name__ == '__main__':
    ...
    parser.add_option("-i", "--idaspecialflag", dest = "ida_flag", action = "store", \
        type = "string", help = "ida special flag", default = "n")
    ...
    if options.ida_flag == "y":
        logging.info("ida_special")
        (comparedTables, insts) = readJmpTablesCompared(mModule2, options.binaryFile, truthTables,True)
    else:
